...
GreenHouse::~GreenHouse() {
  // Si necesitas realizar alguna limpieza específica al destruir un objeto
  // GreenHouse, puedes hacerlo aquí.
}
...
bool GreenHouse::searchUser(std::string name, std::string password,
                            std::string nif) {
  return us_.findUserLogin(ms_->getNameSelectedUser(),
...
void GreenHouse::manageCreateUser() {
  ...
  us_.createUser(ms_->getNameSelectedUser(), ms_->getNIFSelectedUser(),
                 ms_->getPasswordSelectedUser(),
                 ms_->getPrivilegesSelectedUser(), ms_->getEmailSelectedUser());
}
